...
- SQL scripts go in `BackEnd/migrations/` numbered in sequence; each script has a single responsibility and must be idempotent (safe to run more than once — `IF NOT EXISTS` / `ON CONFLICT DO NOTHING` / drop-before-recreate, matching every existing migration's convention).
- **Mandatory migration rule:** any change that touches the database schema — new table, new/altered/dropped column, new index, new constraint, new enum value, seed/reference data change — MUST ship its own numbered migration script in `BackEnd/migrations/` (next sequential number after the highest existing one, e.g. `67_description.sql`) in the same change, never deferred to "later". There is no ORM on Bluehost, but there IS an automated runner (`BackEnd/migrations/runner.js`, invoked from `server.js` before `app.listen()`): it applies every unapplied migration on backend boot, tracked per-schema in `public.schema_migrations`, so a production deploy just needs the backend process to restart (which "Setup Node.js App" already does on every code update) — no manual phpPgAdmin paste required for routine migrations. phpPgAdmin remains the recovery tool if the runner can't apply something. These numbered `.sql` files are still the only source of truth for schema history — skipping one here means the schema history can only be reconstructed by manually diffing the live production database, which is not acceptable. Never edit `tenant_schema.sql` or `hernancius` in place without also emitting a numbered migration for the equivalent change.
- **Mandatory tenant_schema.sql sync rule:** every per-tenant migration (any migration that runs `DO $$ ... FOR r IN SELECT schema_name FROM public.companies ... $$` over tenant schemas, i.e. anything that isn't public-schema-only) MUST also update `BackEnd/templates/tenant_schema.sql` in the SAME change so it reflects the post-migration shape directly — new tables, changed/renamed/dropped columns, new FKs, seed rows for new catalog tables, all of it. `tenant_schema.sql` is executed verbatim to provision every brand-new company (and the runner then seeds that company's `schema_migrations` rows as already-satisfied for every per-tenant migration that exists at creation time, so it never replays historical migrations against a schema that already has their effects baked in — see `companyController.js` `createCompany`); if `tenant_schema.sql` is left stale, new companies are silently born with an out-of-date schema that immediately errors (e.g. `relation "product_types" does not exist`) the moment application code — already updated for the new shape — tries to use it. The migration and the template describe the same end state from two different angles (existing companies vs. new ones) and must never diverge. After editing, sanity-check by diffing the new/changed table definitions in the migration against the corresponding block in `tenant_schema.sql`.
- **Technical debt log:** [TECHNICAL_DEBT.md](TECHNICAL_DEBT.md) tracks deferred work, known gaps, and "for now" decisions that need to be revisited — not a wishlist, only real shortcuts/deferrals with enough context to resume without re-deriving the reasoning. Check it at the start of a session when picking up open-ended or "what's next" work, and whenever a task gets consciously deferred instead of done (a user says "later"/"lo vemos luego", a plan gets partially implemented, a known gap is left on purpose) — add an entry before moving on, don't just remember it verbally. When an open item is resolved, move it to the Closed section instead of deleting it.
