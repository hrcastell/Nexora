...
<template>
  <div class="flex flex-col gap-5 p-3 sm:p-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><button class="flex items-center gap-2 self-start text-sm text-white/55 hover:text-white" @click="router.push({ name: 'hr-employees' })"><ArrowLeft :size="16" /> Volver a empleados</button><button v-if="editable" class="nxr-btn nxr-btn-primary justify-center" :disabled="saving" @click="save"><Save :size="15" /> {{ saving ? 'Guardando...' : 'Guardar cambios' }}</button></div>
    ...
    <template v-else-if="employees.current">
      <section class="flex items-center gap-4 rounded-2xl border border-white/10 p-5" :style="{ background: 'var(--nexora-glass-bg)' }"><div class="flex h-16 w-16 items-center justify-center rounded-full bg-white/10"><UserRound :size="27" class="text-white/40" /></div><div><h1 class="text-xl font-semibold text-white">{{ employees.current.first_name }} {{ employees.current.last_name || '' }}</h1><p class="text-sm text-white/45">Perfil integral del empleado</p></div></section>
...
<style scoped>
label { @apply block text-xs text-white/50; }
.field { @apply mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white outline-none focus:border-white/30 disabled:cursor-not-allowed disabled:opacity-60; }
</style>
