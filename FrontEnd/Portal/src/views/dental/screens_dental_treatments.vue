...
<template>
  <div class="flex flex-col gap-5 p-6">
    <div class="flex items-center justify-between flex-wrap gap-3">
      <h1 class="text-xl font-semibold nxr-text">Tratamientos</h1>
      <button
    ...
    <div v-else-if="store.items.length === 0" class="flex flex-col items-center gap-4 py-20 text-center">
      <Tag :size="48" class="nxr-text-soft" />
      <p class="nxr-text-muted text-sm">No hay tratamientos configurados.</p>
    ...
    <div v-else class="flex flex-col gap-2">
      <!-- Desktop header -->
      <div class="hidden md:grid md:grid-cols-[1fr_100px_100px_80px_120px] gap-4 px-4 py-2 text-xs nxr-text-soft font-semibold uppercase tracking-wide">
        <span>Nombre</span>
      ...
      >
        ...
        <div class="flex-1 min-w-0 md:hidden">
          ...
          <div class="flex items-center gap-2 mt-0.5 text-xs nxr-text-muted">
            <span class="font-semibold nxr-text">{{ fmt(s.final_price) }}</span>
        ...
        <div class="hidden md:grid md:grid-cols-[1fr_100px_100px_80px_120px] gap-4 items-center flex-1">
          <div>
            <p class="text-sm nxr-text truncate">{{ s.name }}</p>
            <p v-if="s.description" class="text-xs nxr-text-muted truncate">{{ s.description }}</p>
          </div>
          <p class="text-sm font-semibold nxr-text text-right">{{ fmt(s.final_price) }}</p>
          <p class="text-xs nxr-text-muted text-center">{{ s.price_mode === 'manual' ? 'Manual' : 'Calculado' }}</p>
          <div class="flex justify-center">
            <button class="nxr-text-muted hover:text-[var(--nexora-text-color)]" @click="toggleActive(s)">
              <ToggleRight v-if="s.is_active" :size="20" class="text-green-400" />
              <ToggleLeft v-else :size="20" />
            </button>
          </div>
          <div class="flex items-center justify-center gap-2">
            <button class="nxr-text-soft hover:text-[var(--nexora-text-color)] transition-colors" @click="openEdit(s)">
              <Pencil :size="14" />
            </button>
            <button class="nxr-text-soft hover:text-red-400 transition-colors" @click="remove(s)">
              <Trash2 :size="14" />
            </button>
          </div>
        </div>
        ...
        <div class="flex items-center gap-2 shrink-0 md:hidden">
          <button class="nxr-text-soft hover:text-[var(--nexora-text-color)]" @click="openEdit(s)"><Pencil :size="14" /></button>
          <button class="nxr-text-muted hover:text-[var(--nexora-text-color)]" @click="toggleActive(s)">
            <ToggleRight v-if="s.is_active" :size="18" class="text-green-400" />
            <ToggleLeft v-else :size="18" />
          </button>
          <button class="nxr-text-soft hover:text-red-400" @click="remove(s)"><Trash2 :size="14" /></button>
        </div>
      </div>
    </div>
    ...
    :draft-state="{ form }">
      <form class="flex flex-col gap-5" @submit.prevent="save">
        ...
        <div class="flex flex-col gap-1.5">
          <label class="text-xs nxr-text-muted">Descripción</label>
          <textarea v-model="form.description" rows="2" class="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm nxr-text outline-none focus:border-white/30 resize-none"></textarea>
        </div>
        <div class="flex flex-col gap-1.5">
          <label class="text-xs nxr-text-muted">Duración estimada (min)</label>
          <input v-model.number="form.estimated_duration_minutes" type="number" min="1" class="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm nxr-text outline-none focus:border-white/30" />
        </div>

        <div class="flex flex-col gap-1.5">
          <label class="text-xs nxr-text-muted">Modo de precio</label>
          <div class="flex gap-2">
            <button
              ...
              :class="form.price_mode === 'manual'
                ...
                : 'nxr-text-muted border-white/10 hover:border-white/30'"
        ...
        <div v-if="form.price_mode === 'manual'" class="flex flex-col gap-1.5">
          <label class="text-xs nxr-text-muted">Precio *</label>
          <input v-model.number="form.manual_price" type="number" min="0" class="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm nxr-text outline-none focus:border-white/30" required />
        </div>
        ...
        <template v-else>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="flex flex-col gap-1.5">
              <label class="text-xs nxr-text-muted">Costo insumos</label>
              <input v-model.number="form.supplies_cost" type="number" min="0" class="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm nxr-text outline-none focus:border-white/30" />
            </div>
            <div class="flex flex-col gap-1.5">
              <label class="text-xs nxr-text-muted">Costo mano de obra</label>
              <input v-model.number="form.labor_cost" type="number" min="0" class="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm nxr-text outline-none focus:border-white/30" />
            </div>
          </div>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="flex flex-col gap-1.5">
              <label class="text-xs nxr-text-muted">IVA (%)</label>
              <input v-model.number="form.tax_rate" type="number" min="0" max="100" class="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm nxr-text outline-none focus:border-white/30" />
            </div>
            <div class="flex flex-col gap-1.5">
              <label class="text-xs nxr-text-muted">Margen ganancia (%)</label>
              <input v-model.number="form.profit_margin" type="number" min="0" class="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm nxr-text outline-none focus:border-white/30" />
            </div>
          </div>
          <div class="px-4 py-3 rounded-xl border border-white/10 flex items-center justify-between" :style="{ background: 'var(--nexora-glass-bg)' }">
            <span class="text-xs nxr-text-muted">Precio estimado</span>
            <span class="text-sm font-bold nxr-text">{{ fmt(calculatedPreview) }}</span>
          </div>
        </template>

        <label v-if="editing" class="flex items-center gap-2 cursor-pointer">
          <input v-model="form.is_active" type="checkbox" class="rounded" />
          <span class="text-sm nxr-text-muted">Activo</span>
        </label>
