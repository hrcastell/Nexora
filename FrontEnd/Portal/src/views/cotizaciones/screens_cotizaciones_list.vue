  ...
  <div class="flex min-h-[calc(100vh-5rem)] flex-col gap-5 p-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="text-xs text-white/40">Cotizaciones a clientes con líneas de stock o tercerizadas.</p>
      </div>
    ...
    <div v-else-if="store.items.length === 0" class="py-16 text-center text-sm text-white/30">Sin cotizaciones.</div>
    <div v-else class="flex flex-col gap-2">
      ...
      >
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p class="text-sm font-semibold text-white">{{ quote.quote_number }}</p>
            <p class="text-xs text-white/40">{{ quote.customer_name?.trim() || 'Sin cliente' }} · {{ fmtDate(quote.created_at) }}</p>
          </div>
          <div class="flex items-center gap-3 sm:justify-end">
            <p class="text-sm font-semibold text-white">{{ fmtMoney(quote.final_amount) }}</p>
