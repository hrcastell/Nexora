...
<template>
  <div class="flex flex-col gap-5 p-3 sm:p-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h1 class="text-xl font-semibold nxr-text">Sesiones de Caja</h1><p class="text-xs nxr-text-muted">Apertura, movimientos y cierre.</p></div><button v-if="canEdit" class="nxr-btn nxr-btn-primary" @click="openPanel"><Plus :size="15" /> Abrir sesi&#243;n</button></div>
    <section v-if="active" class="flex flex-col gap-3 rounded-2xl border border-green-400/20 bg-green-400/5 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p class="text-xs uppercase text-green-300">Sesi&#243;n activa</p><h2 class="mt-1 text-lg font-semibold nxr-text">{{ active.cash_register_name || 'Caja #' + active.cash_register_id }}</h2><p class="text-xs nxr-text-muted">Apertura: {{ money(active.opening_amount) }}</p></div><div v-if="canEdit" class="flex gap-2"><button class="nxr-btn nxr-btn-secondary" @click="movementPanel"><ReceiptText :size="15" /> Movimiento</button><button class="nxr-btn nxr-btn-primary" @click="closePanel"><CheckCircle2 :size="15" /> Cerrar</button></div></section>
    <p v-if="store.error" class="text-sm text-red-400">{{ store.error }}</p>
    <div class="grid grid-cols-1 gap-3 lg:grid-cols-2"><section class="rounded-xl border border-white/10 p-4"><h2 class="mb-3 text-sm font-semibold nxr-text">Sesiones</h2><p v-if="!store.sessions.length" class="py-10 text-center text-sm nxr-text-soft">Sin sesiones.</p><button v-else v-for="session in store.sessions" :key="session.id" class="mb-2 w-full rounded-xl border border-white/10 p-3 text-left hover:bg-white/5" @click="select(session)"><span class="text-sm nxr-text">{{ session.cash_register_name || 'Caja #' + session.cash_register_id }}</span><span class="float-right text-xs nxr-text-muted">{{ session.status === 'open' ? 'Abierta' : 'Cerrada' }}</span></button></section><section class="rounded-xl border border-white/10 p-4"><div v-if="store.selected"><h2 class="text-sm font-semibold nxr-text">Detalle</h2><div v-if="store.selected.status === 'closed'" class="mt-3 grid grid-cols-3 gap-2 text-xs"><div>Esperado<br><strong>{{ money(store.selected.expected_amount) }}</strong></div><div>Contado<br><strong>{{ money(store.selected.counted_amount) }}</strong></div><div>Diferencia<br><strong>{{ money(store.selected.difference_amount) }}</strong></div></div><h3 class="mt-5 text-sm font-semibold nxr-text">Movimientos</h3><p v-if="!store.selected.movements?.length" class="py-8 text-center text-sm nxr-text-soft">Sin movimientos.</p><article v-else v-for="item in store.selected.movements" :key="item.id" class="mt-2 flex justify-between rounded-xl border border-white/10 p-3"><span class="text-sm nxr-text">{{ label(item.movement_type) }}</span><span :class="Number(item.signed_amount) >= 0 ? 'text-green-400' : 'text-red-300'">{{ money(item.signed_amount) }}</span></article></div><p v-else class="py-16 text-center text-sm nxr-text-soft">Seleccion&#225; una sesi&#243;n.</p></section></div>
    <NxrSlidePanel :open="panel === 'open'" title="Abrir sesi&#243;n" size="sm" @close="panel = null"
    draft-key="views/treasury/screens_treasury_cash_sessions.vue#1"
    :draft-entity="'open'"
    :draft-state="{ openForm }"><div class="space-y-3"><select v-model.number="openForm.cash_register_id" class="w-full rounded-xl border border-white/10 bg-white/5 p-2 nxr-text"><option :value="0">Seleccion&#225; una caja</option><option v-for="item in settings.cashRegisters.items" :key="item.id" :value="item.id">{{ item.code }} - {{ item.name }}</option></select><select v-model.number="openForm.employee_id" class="w-full rounded-xl border border-white/10 bg-white/5 p-2 nxr-text"><option :value="0">Seleccion&#225; un cajero</option><option v-for="employee in employees.items" :key="employee.id" :value="employee.id">{{ employeeName(employee) }}</option></select><input v-model.number="openForm.opening_amount" type="number" min="0" placeholder="Monto de apertura" class="w-full rounded-xl border border-white/10 bg-white/5 p-2 nxr-text" /><p v-if="error" class="text-xs text-red-400">{{ error }}</p></div><template #footer><button class="nxr-btn nxr-btn-primary" :disabled="store.saving" @click="saveOpen">Abrir</button></template></NxrSlidePanel>
    <NxrSlidePanel :open="panel === 'movement'" title="Registrar movimiento" size="sm" @close="panel = null"
    draft-key="views/treasury/screens_treasury_cash_sessions.vue#2"
    :draft-entity="active?.id ?? 'movement'"
    :draft-state="{ movement }"><div class="space-y-3"><select v-model="movement.movement_type" class="w-full rounded-xl border border-white/10 bg-white/5 p-2 nxr-text"><option value="sale_in">Ingreso por venta</option><option value="payment_out">Pago emitido</option><option value="deposit_out">Dep&#243;sito</option><option value="withdrawal_out">Retiro</option><option value="adjustment_in">Ajuste de ingreso</option><option value="adjustment_out">Ajuste de salida</option></select><input v-model.number="movement.amount" type="number" min="0" placeholder="Monto" class="w-full rounded-xl border border-white/10 bg-white/5 p-2 nxr-text" /><p class="text-xs nxr-text-muted">El signo se determina autom&#225;ticamente seg&#250;n el tipo de movimiento.</p><textarea v-model="movement.notes" placeholder="Notas" class="w-full rounded-xl border border-white/10 bg-white/5 p-2 nxr-text" /><p v-if="error" class="text-xs text-red-400">{{ error }}</p></div><template #footer><button class="nxr-btn nxr-btn-primary" :disabled="store.saving" @click="saveMovement">Registrar</button></template></NxrSlidePanel>
    <NxrSlidePanel :open="panel === 'close'" title="Cerrar sesi&#243;n" size="sm" @close="panel = null"
    draft-key="views/treasury/screens_treasury_cash_sessions.vue#3"
    :draft-entity="active?.id ?? 'close'"
    :draft-state="{ countedAmount }"
    :draft-setters="{ countedAmount: (value) => countedAmount = value }"><div class="space-y-3"><p class="text-sm nxr-text-muted">El servidor calcula el esperado con todos los movimientos.</p><input v-model.number="countedAmount" type="number" min="0" placeholder="Monto contado" class="w-full rounded-xl border border-white/10 bg-white/5 p-2 nxr-text" /><p v-if="error" class="text-xs text-red-400">{{ error }}</p></div><template #footer><button class="nxr-btn nxr-btn-primary" :disabled="store.saving" @click="saveClose">Confirmar cierre</button></template></NxrSlidePanel>
  </div>
</template>
