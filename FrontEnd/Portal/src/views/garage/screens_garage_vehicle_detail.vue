...
<template>
  <div class="flex flex-col gap-5 p-6 max-w-4xl mx-auto">
    <div class="flex items-center gap-3">
      ...
      <h1 class="text-xl font-semibold text-white flex-1">Detalle del Vehículo</h1>
      <button class="flex items-center gap-2 px-4 py-2 rounded-xl text-sm bg-white/10 text-white hover:bg-white/20" @click="showEdit = true">
        <Edit :size="14" /> Editar
      </button>
    </div>
    ...
    <template v-else-if="store.current">
      <div class="flex items-center gap-4 p-5 rounded-2xl border border-white/10" :style="{ background: 'var(--nexora-glass-bg)' }">
        <Car :size="40" class="text-white/20 shrink-0" />
        <div class="flex-1">
        ...
        <div v-else-if="history">
          <div class="grid grid-cols-3 gap-3 mb-4">
            <div class="p-4 rounded-xl border border-white/10 text-center" :style="{ background: 'var(--nexora-glass-bg)' }">
              ...
              <p class="text-xs text-white/40">Órdenes total</p>
            </div>
            <div class="p-4 rounded-xl border border-white/10 text-center" :style="{ background: 'var(--nexora-glass-bg)' }">
              <p class="text-xl font-bold text-white">{{ history.summary.last_mileage?.toLocaleString() || '—' }}</p>
              <p class="text-xs text-white/40">Último km</p>
            </div>
            <div class="p-4 rounded-xl border border-white/10 text-center" :style="{ background: 'var(--nexora-glass-bg)' }">
              <p class="text-xl font-bold text-white">${{ Math.round(history.summary.total_spent || 0).toLocaleString() }}</p>
              <p class="text-xs text-white/40">Total invertido</p>
            </div>
          </div>
