...
const statusConfig = computed(() => {
  ...
  return               { label: 'Pendiente de pago',      icon: Clock,          color: 'nxr-text-muted',   bg: 'bg-white/5 border-white/10' };
});
</script>
...
<template>
  <div class="flex flex-col gap-5 p-6 max-w-3xl mx-auto">
    <div class="flex items-center gap-3">
      ...
      <h1 class="text-xl font-semibold nxr-text flex-1">Cobros y Pagos</h1>
      <button
    ...
    <template v-else>
      ...
      <div class="p-5 rounded-2xl border" :class="statusConfig.bg">
        <div class="flex items-start justify-between mb-4">
          <div>
            <p class="text-xs nxr-text-muted mb-1">Orden #{{ orderId }}</p>
          ...
          <CreditCard :size="24" class="nxr-text-soft" />
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          <div>
            <p class="text-xs nxr-text-muted mb-1">Total orden</p>
            <p class="text-lg font-bold nxr-text">{{ fmt(summary.total_amount, summary.currency) }}</p>
          </div>
          <div>
            <p class="text-xs nxr-text-muted mb-1">Pagado</p>
            <p class="text-lg font-bold text-green-400">{{ fmt(summary.amount_paid, summary.currency) }}</p>
          </div>
          <div>
            <p class="text-xs nxr-text-muted mb-1">Pendiente</p>
            <p class="text-lg font-bold" :class="summary.amount_pending > 0 ? 'text-yellow-400' : 'nxr-text-soft'">
        ...
        <p class="text-right text-xs nxr-text-soft mt-1">{{ pctPaid }}% pagado</p>
      </div>

      <!-- New payment form -->
      <div v-if="showForm" class="p-5 rounded-2xl border border-white/10" :style="{ background: 'var(--nexora-glass-bg)' }">
        <h3 class="text-sm font-semibold nxr-text mb-4">Registrar nuevo pago</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label class="block text-xs nxr-text-muted mb-1">Monto *</label>
            <input
              ...
              class="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 nxr-text text-sm outline-none focus:border-white/40"
            />
          </div>
          <div>
            <label class="block text-xs nxr-text-muted mb-1">Método de pago</label>
            <select v-model="form.payment_method" class="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 nxr-text text-sm outline-none">
          ...
          <div>
            <label class="block text-xs nxr-text-muted mb-1">Fecha</label>
            <input v-model="form.payment_date" type="date" class="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 nxr-text text-sm outline-none focus:border-white/40" />
          </div>
          <div>
            <label class="block text-xs nxr-text-muted mb-1">Referencia / Folio</label>
            <input v-model="form.reference" type="text" placeholder="Nro. de transacción, folio..." class="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 nxr-text text-sm outline-none focus:border-white/40" />
          </div>
          <div class="col-span-2">
            <label class="block text-xs nxr-text-muted mb-1">Notas</label>
            <input v-model="form.notes" type="text" class="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 nxr-text text-sm outline-none focus:border-white/40" />
          </div>
        </div>
        <p v-if="error" class="mt-2 text-xs text-red-400">{{ error }}</p>
        <div class="flex justify-end gap-2 mt-4">
          <button type="button" class="px-4 py-2 rounded-xl text-sm nxr-text-muted hover:text-[var(--nexora-text-color)]" @click="showForm = false">Cancelar</button>
          <button
            type="button"
      ...
      <!-- Payments list -->
      <div>
        <p class="text-xs nxr-text-muted mb-3">{{ payments.length }} pago{{ payments.length !== 1 ? 's' : '' }} registrado{{ payments.length !== 1 ? 's' : '' }}</p>

        <div v-if="payments.length === 0" class="text-center nxr-text-soft py-12 text-sm rounded-xl border border-dashed border-white/10">
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                ...
                <span class="text-xs px-2 py-0.5 rounded-full bg-white/10 nxr-text-muted">{{ METHOD_LABELS[p.payment_method ?? 'otro'] ?? p.payment_method }}</span>
              </div>
