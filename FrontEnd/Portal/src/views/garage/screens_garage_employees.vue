...
<template>
  <div class="flex flex-col gap-5 p-6">
    <div class="flex items-center justify-between">
      <h1 class="text-xl font-semibold nxr-text">Empleados / Mecánicos</h1>
    ...
    <div class="flex items-center gap-3">
      <div class="flex-1 relative">
        ...
        <input v-model="q" type="text" placeholder="Buscar por nombre o especialidad..." class="w-full pl-9 pr-4 py-2 rounded-xl bg-white/5 border border-white/10 text-sm nxr-text outline-none focus:border-white/30" />
      </div>
    ...
    <div v-else-if="store.items.length === 0" class="text-center nxr-text-soft py-16 text-sm">No se encontraron empleados.</div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-3">
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium nxr-text truncate">{{ e.first_name }} {{ e.last_name || '' }}</p>
          <p class="text-xs nxr-text-muted truncate">{{ e.role_name || '' }}{{ e.specialty ? ` · ${e.specialty}` : '' }}</p>
        </div>
        <div class="flex items-center gap-2 shrink-0">
          <button type="button" class="nxr-text-soft hover:text-[var(--nexora-text-color)]" @click="toggleStatus(e)">
            <ToggleRight v-if="e.status === 'active'" :size="18" class="text-green-400" />
            <ToggleLeft v-else :size="18" />
          </button>
          <button type="button" class="nxr-text-soft hover:text-[var(--nexora-text-color)]" @click="openEdit(e)">
