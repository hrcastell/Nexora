...
<template>
  <div class="flex flex-col gap-5 p-6">
    <div class="flex items-center justify-between">
      <h1 class="text-xl font-semibold nxr-text">Vehículos</h1>
    ...
    <div class="flex items-center gap-3">
      <div class="flex-1 relative">
        ...
        <input v-model="q" type="text" placeholder="Buscar por placa, marca, modelo..." class="w-full pl-9 pr-4 py-2 rounded-xl bg-white/5 border border-white/10 text-sm nxr-text outline-none focus:border-white/30" />
      </div>
    ...
    <div v-else-if="store.items.length === 0" class="text-center nxr-text-soft py-16 text-sm">No se encontraron vehículos.</div>
    ...
    <div v-else class="flex flex-col gap-2">
      ...
      >
        <Car :size="18" class="nxr-text-soft shrink-0" />
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium nxr-text">{{ (v as any).brand || '' }} {{ (v as any).model || '' }} {{ v.version || '' }}</p>
          <p class="text-xs nxr-text-muted truncate">{{ v.plate || 'Sin placa' }} · {{ v.year || '' }} · {{ (v as any).customer_name || '' }}</p>
        </div>
        <div class="flex items-center gap-2 shrink-0">
          <button type="button" class="nxr-text-soft hover:text-[var(--nexora-text-color)]" @click.stop="toggleStatus(v.id, v.status)">
