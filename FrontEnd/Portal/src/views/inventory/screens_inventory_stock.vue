...
<template>
  <div class="flex flex-col gap-5 p-6">
    <div>
      <h1 class="text-xl font-semibold nxr-text">Stock</h1>
      <p class="text-xs nxr-text-muted">Consulta de existencias por producto o bodega.</p>
    </div>
    ...
    <div class="grid grid-cols-1 gap-3 rounded-2xl border border-white/10 p-4 md:grid-cols-3" :style="{ background: 'var(--nexora-glass-bg)' }">
      <div>
        <label class="mb-1 block text-xs nxr-text-muted">Vista</label>
        <select v-model="mode" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm nxr-text">
      ...
      <div v-if="mode === 'product'" class="md:col-span-2">
        ...
        <select v-model.number="productId" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm nxr-text">
          <option :value="undefined">Todos</option>
          <option v-for="product in productsStore.items" :key="product.id" :value="product.id">{{ product.name }}</option>
        </select>
      </div>
      <div v-else class="md:col-span-2">
        <label class="mb-1 block text-xs nxr-text-muted">Bodega</label>
        <select v-model.number="warehouseId" class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm nxr-text">
    ...
    <div v-else-if="stockStore.rows.length === 0" class="rounded-2xl border border-white/10 py-16 text-center text-sm nxr-text-soft">
      <Search class="mx-auto mb-3 h-8 w-8 nxr-text-soft" />
    ...
    <div v-else class="flex flex-col gap-2">
      <div v-for="row in stockStore.rows" :key="`${row.product_id}-${row.warehouse_id}`" class="grid grid-cols-1 gap-2 rounded-xl border border-white/10 p-4 sm:grid-cols-[1fr_1fr_auto] sm:items-center" :style="{ background: 'var(--nexora-glass-bg)' }">
        <div><p class="text-sm font-medium nxr-text">{{ row.product_name }}</p><p class="text-xs nxr-text-soft">Producto #{{ row.product_id }}</p></div>
        <div><p class="text-sm nxr-text-muted">{{ row.warehouse_name }}</p><p class="text-xs nxr-text-soft">Bodega #{{ row.warehouse_id }}</p></div>
        <p class="text-lg font-semibold nxr-text">{{ Number(row.on_hand || 0).toLocaleString('es-CL') }}</p>
      </div>
    </div>
  </div>
</template>
