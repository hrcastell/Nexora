...
<template>
  <div class="flex flex-col gap-3">
    <div>
      ...
      <div class="relative">
        <div class="relative flex items-center">
          <User :size="14" class="absolute left-3 nxr-text-soft" />
          <input
            v-model="customerQuery"
            type="text"
            :disabled="disabled"
            class="w-full pl-8 pr-8 py-2 rounded-xl bg-white/5 border border-white/10 nxr-text text-sm outline-none focus:border-white/40 transition-colors"
            placeholder="Buscar cliente..."
            @input="onCustomerInput"
            @focus="customerOpen = true"
          />
          <button v-if="selectedCustomer" type="button" class="absolute right-2 nxr-text-soft hover:text-[var(--nexora-muted-text)]" @click="clearCustomer">✕</button>
        </div>
        ...
        >
          ...
          >
            <div class="w-7 h-7 rounded-full overflow-hidden bg-white/10 flex items-center justify-center">
              ...
              <User v-else :size="14" class="nxr-text-soft" />
            </div>
            <div>
              <p class="text-sm nxr-text">{{ c.first_name }} {{ c.last_name || '' }}</p>
              <p class="text-xs nxr-text-muted">{{ c.email || c.phone || '' }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="selectedCustomer && vehicleResults.length > 0">
      <label class="block text-xs nxr-text-muted mb-1">Vehículo</label>
      <div class="relative">
        <button
          type="button"
          :disabled="disabled"
        ...
        >
          <div class="flex items-center gap-2">
            <Car :size="14" class="nxr-text-soft" />
            <span :class="selectedVehicle ? 'nxr-text' : 'nxr-text-muted'">{{ vehicleLabel || 'Seleccionar vehículo...' }}</span>
          </div>
          <ChevronDown :size="14" class="nxr-text-muted" :class="vehicleOpen ? 'rotate-180' : ''" />
        </button>
        ...
        >
          ...
          >
            <Car :size="14" class="nxr-text-muted" />
            <div>
              <p class="text-sm nxr-text">{{ v.plate || 'Sin placa' }} — {{ (v as any).brand || '' }} {{ (v as any).model || '' }}</p>
              <p class="text-xs nxr-text-muted">{{ v.year || '' }} {{ (v as any).vehicle_type || '' }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="selectedCustomer && vehicleResults.length === 0" class="text-xs nxr-text-muted italic">
