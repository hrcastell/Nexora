...
<template>
  <section aria-labelledby="setup-checklist-title" class="rounded-2xl border border-white/10 bg-white/[.035] p-4 sm:p-5">
    <h2 id="setup-checklist-title" class="text-base font-semibold text-white">{{ title }}</h2>
    <ol class="mt-4 space-y-3">
      <li v-for="(item, index) in items" :key="item.id" class="flex gap-3 rounded-xl border border-white/10 p-3">
        ...
        <div class="min-w-0 flex-1"><p class="font-medium text-white">{{ item.title }}</p><p class="mt-1 text-sm leading-5 text-white/60">{{ item.description }}</p></div>
        <button v-if="!item.complete && item.actionLabel" type="button" class="min-h-11 shrink-0 self-center rounded-xl border border-white/15 px-3 text-xs font-medium text-white hover:bg-white/10" @click="$emit('navigate', item)">{{ item.actionLabel }}</button>
      </li>
    </ol>
  </section>
</template>
